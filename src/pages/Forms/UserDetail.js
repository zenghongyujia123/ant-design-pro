      ...
      >
        <Card bordered={false}>
          <Form onSubmit={this.handleSubmit} hideRequiredMark style={{ marginTop: 8 }}>
            <FormItem {...formItemLayout} label='用户名'>{getFieldDecorator('username', { initialValue: data.username || '' })(<Input readOnly placeholder='无' />)}</FormItem>
            ...
            <FormItem {...formItemLayout} label='银行卡用户名'>{getFieldDecorator('bank_username', { initialValue: data.bank_username || '' })(<Input readOnly placeholder='无' />)}</FormItem>
            <FormItem {...formItemLayout} label='银行卡号'>{getFieldDecorator('bank_cardno', { initialValue: data.bank_cardno || '' })(<Input readOnly placeholder='无' />)}</FormItem>
            <FormItem {...formItemLayout} label='身份证号'>{getFieldDecorator('id_no', { initialValue: data.face_info ? data.face_info.id_no : '' })(<Input readOnly placeholder='无' />)}</FormItem>


            <FormItem {...formItemLayout} label='乙方渠道'>{getFieldDecorator('parent', { initialValue: data.parent || '' })(<Input readOnly placeholder='无' />)}</FormItem>
            ...
            <Divider>首次次退款申请相关 （总状态：{data.first_refund_status}）(提交时间：{moment(data.first_refund_time).format('YYYY-MM-DD HH:mm:ss')})</Divider>
            {
              data.first_refund_list && data.first_refund_list.length > 0 && data.first_refund_list.map((first_refund, index) => {
                return <FormItem key={first_refund.jiafang_id} {...formItemLayout} label={`${first_refund.name}`}>
                  ...
                  <img style={{ width: '60px', height: '60px', marginRight: '10px' }} onClick={e => window.open(first_refund.refund_image.replace('pjgg8ntmh.bkt.clouddn.com','image.chaoqianwang.com'))} src={first_refund.refund_image.replace('pjgg8ntmh.bkt.clouddn.com','image.chaoqianwang.com')}></img>
                  {/* submit,pass,unpass */}
            ...
            <Divider>二次退款申请相关 （总状态：{data.refund_status}）(提交时间：{moment(data.refund_time).format('YYYY-MM-DD HH:mm:ss')})</Divider>
            {
              ...
              data.refund_list && data.refund_list.length > 0 && data.refund_list.map((refund, index) => {
                return <FormItem key={refund.jiafang_id} {...formItemLayout} label={`${refund.name}`}>
                  ...
                  <img style={{ width: '60px', height: '60px', marginRight: '10px' }} onClick={e => window.open(refund.refund_image.replace('pjgg8ntmh.bkt.clouddn.com','image.chaoqianwang.com'))} src={refund.refund_image.replace('pjgg8ntmh.bkt.clouddn.com','image.chaoqianwang.com')}></img>
                  {/* submit,pass,unpass */}
